...
export class HomeService {
  ...
  fetch_images$(page: Pageable): Observable<FileHandle[]> {
    return this.http.get<ImageResponse[]>(this.HOST + "api/v1/image/all", {
      headers: { 'content-type': 'application/json' },
      params: { 'page': page.page, 'size': page.size },
    }).pipe(map((arr: ImageResponse[]) => arr.map((res: ImageResponse) => {
      if (res === null) {
        res = {
          name: '',
          media_type: '',
      ...
      return this.fileService.createImageFromByte(res);
    })));
  }
